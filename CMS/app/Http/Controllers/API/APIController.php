...
class APIController extends Controller
{
    ...
    public function signup(Request $request){
        ...
        if($firstname == ''){
            $data = array('code'=>0, 'data'=>'First name field is not exist.');
            return \GuzzleHttp\json_encode($data);
        }else if($lastname == ''){
            $data = array('code'=>0, 'data'=>'Last name field is not exist.');
            return \GuzzleHttp\json_encode($data);
        }else if($password == ''){
            $data = array('code'=>0, 'data'=>'Password field is not exist.');
            return \GuzzleHttp\json_encode($data);
        }else if($email == ''){
            $data = array('code'=>0, 'data'=>'Email field is not exist.');
            return \GuzzleHttp\json_encode($data);
        }
        if(strpos($email, '@') === false){
            $data = array('code'=>0, 'data'=>'Email format is not right.');
            return \GuzzleHttp\json_encode($data);
        }else{
            $email_exist = DB::table('users')->where('email',$email)->first();
            if(!empty($email_exist)){
                $data = array('code'=>0, 'data'=>'Email already exist. Use another email.');
                return \GuzzleHttp\json_encode($data);
            }
        }
        ...
        if($file = $request->file('profile')) {
            ...
            $safeName = 'avatar_'.time() . '.' . $extension;
            $file->move($destinationPath, $safeName);
            //$photopath = 'http://app.dressd.us/img/avatar/'.$safeName;
            $photopath = 'http://192.168.1.240:9003/img/avatar/'.$safeName;
        }else{
        ...
        $id = rand(10, 99).time().rand(10, 99);
        $rand = substr(md5(microtime()),rand(0,26),5).rand(10,99);
        $rand = base64_encode($rand);
        DB::table('users')->insert(['name' => $id, 'first_name' => $firstname, 'last_name' => $lastname, 'email' => $email, 'token' => $rand, 'activated'=>1, 'password' => $password, 'avatar' => $photopath, 'created_at'=>date('Y-m-d H:i:s')]);
        $user_profile = DB::table('profiles')->where('user_id',$id)->first();
        if(empty($user_profile)) {
            DB::table('profiles')->insert(['user_id' => $id, 'mynotification' => 1, 'friendnotification' => 1]);
        }else {
            DB::table('profiles')->where('user_id', $id)->update(['mynotification' => 1, 'friendnotification' => 1]);
        }
        $profile = DB::table('profiles')->where('user_id', $id)->select('mynotification', 'friendnotification')->first();
        $userstyles = DB::table('userstyles')->where('user_id', $id)->select('style_id')->get();
        $userstyle1 = '';
        ...
        $pass = DB::table('users')->where('name',$id)->first()->password;
        $data = array('code'=>1, 'data'=>'Successfully authorized.', 'profile'=>$profile,'avatar'=>$photopath, 'user_id'=>$id, '_token'=>$rand, 'userstyles'=>$userstyle1);
        return \GuzzleHttp\json_encode($data);
    ...
    public function signin(Request $request){
        ...
        $password = $request->get('password', '');
        if($password == ''){
            $data = array('code'=>0, 'data'=>'Password field field is not exist.');
            return \GuzzleHttp\json_encode($data);
        }else if($email == ''){
            $data = array('code'=>0, 'data'=>'Email field is not exist.');
            return \GuzzleHttp\json_encode($data);
        ...
        $password = base64_encode($password);
        $user = \DB::table('users')->where('email', $email)->where('password', $password)->where('activated', 1)->first();
        ...
        if(!empty($user)){
            ...
            $rand = substr(md5(microtime()),rand(0,26),5).rand(10,99);
            $rand = base64_encode($rand);
            \DB::table('users')->where('name', $id)->where('email', $email)->update(['token'=>$rand]);
            $profile = DB::table('profiles')->where('user_id', $id)->select('mynotification', 'friendnotification')->first();
            $userstyles = DB::table('userstyles')->where('user_id', $id)->select('style_id')->get();
            $userstyle1 = '';
            foreach($userstyles as $userstyle){
            ...
            $data = array('code'=>1, 'data'=>'Successfully authorized.', 'profile'=>$profile,'firstname'=>$firstname,'lastname'=>$lastname,'avatar'=>$avatar, 'user_id'=>$id, '_token'=>$rand, 'userstyles'=>$userstyle1);
            return \GuzzleHttp\json_encode($data);
        }else{
            $data = array('code'=>0, 'data'=>'User is not exist.');
            return \GuzzleHttp\json_encode($data);
        }
    ...
    public function sendverifycode(Request $request){
        ...
        if(strpos($email, '@') === false){
            ...
            $email_exist = DB::table('users')->where('email',$email)->where('activated', 1)->first();
            if(empty($email_exist)){
        ...
        \DB::table('password_resets')->insert(['email'=>$email, 'token'=>$verifycode]);
        try {
            $ch = array();
            $j = 0;
            $mh = curl_multi_init();
            $subject = "Verify code for forgot password";//$request->get('subject', '');
            $content = "Hi, there!<br><br>
                       You requested a verification code in order to change your Dress'd password. The verification code is shown below:<br><br><br>
                       ".$verifycode."<br><br><br>
                       We hope you get back to sharing all of your outfits soon! Don't hesitate to contact us if you are having any additional problems logging in to your Dress'd account.<br><br>
                       Thank you<br><br>
                       The Dress'd Team";
            //$content = base64_encode($content);
            //$subject = base64_encode($subject);
            $data = array('content'=>$content, 'subject'=>$subject, 'email'=>$email);
            $data1 = array();
            $data1[] = $data;
            $finaldata = array('data'=>json_encode($data1, JSON_UNESCAPED_UNICODE));
            $ch[$j] = curl_init();
            //echo $_SERVER['HTTP_HOST'];return;
            // set URL and other appropriate options
            curl_setopt($ch[$j], CURLOPT_URL, $_SERVER['HTTP_HOST'] . "/mail/test/mail.php");
            //curl_setopt($ch[0], CURLOPT_URL, public_path() . "/mail/test/mail.php");
            curl_setopt($ch[$j], CURLOPT_HEADER, 0);
            curl_setopt($ch[$j], CURLOPT_POST, true);
            curl_setopt($ch[$j], CURLOPT_POSTFIELDS, $finaldata);
            ...
            $data = array('code'=>1, 'data'=>'Sent verify code. Please check email.');
            return \GuzzleHttp\json_encode($data);

        }catch(Exception $e){
            $data = array('code'=>0, 'data'=>'Failed verify code.');
            return \GuzzleHttp\json_encode($data);
        }
    }
    public function changepassword(Request $request){
        ...
        if($email == ''){
            ...
            $data = array('code'=>0, 'data'=>'Verify code field is not exist.');
            return \GuzzleHttp\json_encode($data);
        }else if($newpassword == ''){
            $data = array('code'=>0, 'data'=>'New password field is not exist.');
            return \GuzzleHttp\json_encode($data);
        }
        if(strpos($email, '@') === false){
            $data = array('code'=>0, 'data'=>'Email format is not right.');
        ...
        $verifycode_state = \DB::table('password_resets')->where('email',$email)->where('token',$verifycode)->first();

        if(!empty($verifycode_state)){
            \DB::table('password_resets')->where('email', $email)->delete();

            $newpassword = base64_encode($newpassword);
            $rand = substr(md5(microtime()),rand(0,26),5).rand(10,99);
            $rand = base64_encode($rand);
            \DB::table('users')->where('email', $email)->where('activated', 1)->update(['token'=>$rand,'password'=>$newpassword]);

            $user = \DB::table('users')->where('email', $email)->where('password', $newpassword)->where('activated', 1)->first();
            if(!empty($user)){
                $firstname = $user->first_name;
                $lastname = $user->last_name;
                $id = $user->name;
            ...
            $data = array('code'=>0, 'data'=>'Verify code is not right.');
            return \GuzzleHttp\json_encode($data);
        }
    }
    public function authorization(Request $request){
        ...
        if($fbid == ''){
            ...
            $user = \DB::table('users')->where('name', $fbid)->where('email',$email)->where('email',$email)->first();
            if (empty($user)) {
                $user1 = \DB::table('users')->where('email',$email)->first();
                if(!empty($user1)){
                    //user exist
                    $data = array('code'=>0, 'data'=>'User already exist.');
                    return \GuzzleHttp\json_encode($data);
                ...
                $user_profile = DB::table('profiles')->where('user_id',$fbid)->first();
                if(empty($user_profile)) {
                    DB::table('profiles')->insert(['user_id' => $fbid, 'mynotification' => 1, 'friendnotification' => 1]);
                }else {
                    DB::table('profiles')->where('user_id', $fbid)->update(['mynotification' => 1, 'friendnotification' => 1]);
                }
    ...
    public function avatarupdate(Request $request){
        ...
        if($_token == ''){
            ...
            $data = array('code'=>0, 'data'=>'User id is not exist.');
            return \GuzzleHttp\json_encode($data);
        }else{
            ...
            if(empty($user)){
                ...
                if($file = $request->file('profile')) {
                    ...
                    DB::table('users')->where('name', $user_id)->where('token', $_token)->where('activated', 1)
                        ->update(['avatar'=>$photopath]);

                    $data = array('code'=>1, 'data'=>'Successfully Updated', 'avatar'=>$photopath);
                    return \GuzzleHttp\json_encode($data);
                }else{
                    $data = array('code'=>0, 'data'=>'There is not the photo.');
                    return \GuzzleHttp\json_encode($data);
   ...
   public function accepttofriend(Request $request){
        ...
        if($_token == ''){
            ...
            if(empty($user)){
                ...
                if(!empty($invite)){
                    ...
                    $content = $user->first_name.' '.$user->last_name.' accepted you as a friend.';
                    ...
                    //$body = array('user_id'=>$myfb_id, 'first_name'=>$user->first_name, 'last_name'=>$user->last_name, 'avatar'=>$user->avatar);
                    $title = 'Notification';
                    $body = $user->first_name.' '.$user->last_name.' accepted you as a friend.';
    ...
    public function friendlist(Request $request){
        ...
        if($_token == ''){
            ...
            if(empty($user)){
                ...
                $frienduser = DB::table('users')->where('name', $friend_id)->where('activated', 1)->first();
                if(empty($frienduser)){
                    $data = array('code'=>0, 'data'=>'No friend.');
                    return \GuzzleHttp\json_encode($data);
                }
                $styles = DB::table('friends as i')
                    ->join('users as u', 'u.name', '=', 'i.friend_id')
    ...
    public function getalluserwithfb(Request $request){
        $_token = $request->get('_token', '');
        $user_id = $request->get('user_id', '');
        $fb_friendids = $request->get('fb_friendids', '');
        if($_token == ''){
            $data = array('code'=>0, 'data'=>'Failed token.');
            return \GuzzleHttp\json_encode($data);
        }else{
            ...
            if(empty($user)){
                ...
                $fbfriendids = explode(',', $fb_friendids);
...
//                $fbfriendids1 = array();
//                foreach ($fbfriendids as $fbfriendid)
//                    if (!in_array($fbfriendid, $friends))
//                        $fbfriendids1[] = $fbfriendid;

                $user_fbfriend = DB::table('users')
                    ->select(['name', 'first_name', 'last_name', 'avatar','signup_ip_address as relation'])
                    ->whereIn('name', $fbfriendids)
                    ->whereNotIn('name', $friends)
                    ->where('activated', 1)
                    ->orderby('first_name', 'asc')->get();
                $user_nofbfriend = DB::table('users')
                    ->select(['name', 'first_name', 'last_name', 'avatar','signup_ip_address as relation'])
                    ->where('name', '!=', $user_id)
                    ->whereNotIn('name', $fbfriendids)
                    ->whereNotIn('name', $friends)
                    ->where('activated', 1)
                    ->orderby('first_name', 'asc')->get();
                $users = array_merge($user_fbfriend->toArray(), $user_nofbfriend->toArray());
                ...
                foreach ($users as $user){
                    ...
                    {
                        $invite = DB::table('notifications')->where('user_id',$user->name)->where('friend_id',$user_id)->where('type_id',2)->first();
                        $invite1 = DB::table('notifications')->where('user_id',$user_id)->where('friend_id',$user->name)->where('type_id',2)->first();
                        if(empty($invite) && empty($invite1)){
                            $user->relation = "none";
                        }
                        if(!empty($invite) && empty($invite1)){
    ...
    public function addpost(Request $request){
        ...
        $noexpire = $request->get('noexpire', 'no');
        $createdtime = time();
    ...
    public function addlike1(Request $request){
        $user_id = $request->get('user_id', '');
        $_token = $request->get('_token', '');
        $post_id = $request->get('post_id', '');
            ...
            if(empty($user)){
                ...
                if(!empty($post)) {
                    if (!empty($like)) {
                        ...
                        \DB::table('posts')->where('id', $post_id)->update(['like1'=>$post_like1, 'like2'=>$post_like2, 'like3'=>$post_like3, 'like4'=>$post_like4, 'like5'=>$post_like5, 'like6'=>$post_like6, 'like7'=>$post_like7, 'like8'=>$post_like8]);
                        \DB::table('likes')->where('post_id', $post_id)->where('user_id', $user_id)->update(['like1' => $likeval1, 'like2' => $likeval2, 'like3' => $likeval3, 'like4' => $likeval4, 'like5' => $likeval5, 'like6' => $likeval6, 'like7' => $likeval7, 'like8' => $likeval8]);

                        $result = array('like1' => strval($post_like1),'like2' => strval($post_like2),'like3' => strval($post_like3),'like4' => strval($post_like4),'like5' => strval($post_like5),'like6' => strval($post_like6),'like7' => strval($post_like7),'like8' => strval($post_like8),'me1' => strval($likeval1),'me2' => strval($likeval2),'me3' => strval($likeval3),'me4' => strval($likeval4),'me5' => strval($likeval5),'me6' => strval($likeval6),'me7' => strval($likeval7),'me8' => strval($likeval8) );
                        ...
                        \DB::table('posts')->where('id', $post_id)->update(['like1'=>$post_like1, 'like2'=>$post_like2, 'like3'=>$post_like3, 'like4'=>$post_like4, 'like5'=>$post_like5, 'like6'=>$post_like6, 'like7'=>$post_like7, 'like8'=>$post_like8]);
                        \DB::table('likes')->insert(['user_id' => $user_id, 'post_id' => $post_id, 'like1' => $likeval1, 'like2' => $likeval2, 'like3' => $likeval3, 'like4' => $likeval4, 'like5' => $likeval5, 'like6' => $likeval6, 'like7' => $likeval7, 'like8' => $likeval8]);
            ...
            if(empty($user)){
                ...
                if(!empty($post)) {
                    if (!empty($like)) {
                        ...
                        $oldlikeval1 = 0; $oldlikeval2 = 0; $oldlikeval3 = 0;
                        $oldlikeval4 = 0; $oldlikeval5 = 0; $oldlikeval6 = 0;
                        $oldlikeval7 = 0; $oldlikeval8 = 0; $oldlikeval9 = 0;
                        $oldlikeval10 = 0; $oldlikeval11 = 0; $oldlikeval12 = 0;
    ...
    public function getallactivepost(Request $request){
        ...
        if($_token == ''){
            ...
            if(empty($user)){
                ...
                $friend1 = \DB::table('friends')->join('users as u', 'u.name', '=', 'friend_id')->where('user_id', $user_id)->where('status',1)->select('friend_id')->get();
                $friends = array();
    ...
    public function getallactivepostwithstyle(Request $request){
        ...
        if($_token == ''){
            ...
            if(empty($user)){
                ...
                $block = \DB::table('block')->join('users as u', 'u.name', '=', 'friend_id')->select('friend_id')->where('user_id', $user_id)->get();
                $blockByUser = \DB::table('block')->join('users as u', 'u.name', '=', 'user_id')->select('user_id')->where('friend_id', $user_id)->get();
                
                $post_public1 = \DB::table('profiles')->join('users as u', 'u.name', '=', 'user_id')->where('public', 1)->select('user_id')->get();
                $friend1 = \DB::table('friends')->join('users as u', 'u.name', '=', 'friend_id')->where('user_id', $user_id)->where('status',1)->select('friend_id')->get();
        ...
        if($_token == ''){
            ...
            if(empty($user)){
                ...
                $notifications = \DB::table('notifications as n')
                    ...
                    ->where('n.friend_id', $user_id)->orderby('n.id', 'desc')->offset((int)$start_num)->limit((int)$num)->get();
